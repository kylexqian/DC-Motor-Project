...
void currentcontrol_init(void){
  // Initialize Timer2 interrupt @ 5 kHz (current control loop)
  ...
  IPC2bits.T2IP = 5;       // priority for interrupt
  IFS0bits.T2IF = 0;       // clear interrupt flag
  IEC0bits.T2IE = 1;       // enable interrupt
  T2CONbits.ON = 1;        // turn on Timer2

  // Initialize Timer3 and OutputCompare1 @ 20 kHz (Pulse Width Modulator)
  ...
  // Initialize digital output pin to control motor direction (H-bridge)
